...
const setProfilePicCommand = require('./commands/setpp');
    ...
    try {
        ...
        switch (true) {
            ...
            case userMessage.startsWith('.autostatus'):
                ...
                break;
            case userMessage === '.grouplist' || userMessage === '.listgroups':
            ...
            case userMessage.startsWith('.join'):
                // Get the original message text without converting to lowercase
                const originalMessageText = message.message?.conversation ||
                    message.message?.extendedTextMessage?.text || '';
                
                // Pass the original text to preserve case sensitivity in the invite link
                await joinGroupCommand(sock, chatId, senderId, originalMessageText);
                break;
            case userMessage.startsWith('.leave'):
                await leaveGroupCommand(sock, chatId, senderId);
                break;
            ...
            case userMessage.startsWith('.broadcast') || userMessage.startsWith('.bc'):
                ...
                break;
            case userMessage.startsWith('.setgroupicon') || userMessage.startsWith('.seticon') || userMessage.startsWith('.gpicon'):
            ...
            case userMessage.startsWith('.setdesc') || userMessage.startsWith('.desc') || userMessage.startsWith('.setdescription'):
                // Get the original message text to preserve formatting in the description
                const originalDescText = message.message?.conversation ||
                    message.message?.extendedTextMessage?.text || '';
                
                // Pass the original text to preserve formatting in the group description
                await setDescriptionCommand(sock, chatId, senderId, originalDescText);
                break;
