...
let phoneNumber = "62881037442853"
...
async function startXeonBotInc() {
    ...
    if (pairingCode && !useMobile) {
        ...
        if (!!global.phoneNumber) {
            ...
            phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😍\nFor example: 62881037442853 : `)))
        }

        phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
        
        // HAPUS timeout di sini, pindahkan ke dalam event connection.update
    }
    ...
    XeonBotInc.ev.on('connection.update', async (s) => {
        ...
        // Tampilkan QR code jika tersedia dan pairing code tidak digunakan
        if (qr && !pairingCode) {
            console.log(chalk.yellow('Scan QR code to connect...'))
        }
        
        // Tambahkan kondisi ini untuk meminta pairing code saat koneksi "connecting"
        if (connection === "connecting" && pairingCode && !useMobile) {
            try {
                ...
                console.log(chalk.red('Failed to request pairing code. Please try again or use QR code method.'))
            }
        }
        ...
        if (
            ...
            console.log(chalk.red(`Connection closed with status ${statusCode}: ${reason}`))
            
            // Jangan restart jika logout
            if (statusCode !== 401) {
                console.log(chalk.yellow('Reconnecting...'))
                setTimeout(startXeonBotInc, 3000)
            } else {
                console.log(chalk.red('Logged out. Please delete the session folder and restart.'))
                process.exit(0)
            }
        }
    })
