...
async function startXeonBotInc() {
    ...
    // Tanya metode login jika belum ada sesi dan tidak ada flag --pairing-qr
    let loginMethod = pairingQR ? "2" : "1" // Set ke QR jika flag ada
    if (!existsSync('./session') && !pairingQR) {
        loginMethod = await askLoginMethod()
    }
    ...
    if (!XeonBotInc.authState.creds.registered) {
        if (loginMethod === "1" && !pairingQR) {  // Tambahkan pengecekan !pairingQR
            ...
            // QR Code method - langsung tampilkan QR
            console.log(chalk.yellow('\nMenginisialisasi koneksi dengan QR Code...'))
            console.log(chalk.cyan('\nSilakan scan QR code berikut menggunakan WhatsApp di perangkat anda:'))
            // QR akan otomatis ditampilkan karena printQRInTerminal: true
        }
    }
